...
export class Opcode {
  ...
  isMemoryAccess(): OpcodeMemoryAccess {
    switch ((_Opcodes[this.mnemonic.toLowerCase()] || { memory: null }).memory) {
      case 'read':
        return OpcodeMemoryAccess.read;
      case 'write':
        return OpcodeMemoryAccess.write;
      case 'full':
        return OpcodeMemoryAccess.full;
    }
  }
...
const _Opcodes: { [name: string]: _Opcode } = {
  ...
  calldataload: {
    ...
    delta: 1,
    alpha: 1,
    doc: 'calldataWordValue = calldataload(byteOffet)',
  },
  ...
  calldatacopy: {
    ...
    delta: 3,
    alpha: 0,
    doc: 'calldatacopy(dstMemoryIndex, dataIndex, length)',
    memory: 'write',
  },
  ...
  codecopy: {
    ...
    delta: 3,
    alpha: 0,
    doc: 'codecopy(dstMemoryIndex, codeIndex, length)',
    memory: 'write',
  },
  ...
  extcodecopy: {
    ...
    delta: 4,
    alpha: 0,
    doc: 'extcodecopy(address, dstMemoryIndex, extcodeIndex, length)',
    memory: 'write',
  },
  ...
  returndatacopy: {
    ...
    delta: 3,
    alpha: 0,
    doc: 'returndatacopy(dstMemoryOffset, returndataIndex, length)',
    memory: 'write',
  },
  ...
  mload: {
    ...
    delta: 1,
    alpha: 1,
    doc: 'memoryWordValue = mload(memoryByteIndex)',
    memory: 'read',
  },
  mstore: {
    value: 0x52,
    delta: 2,
    alpha: 0,
    doc: 'mstore(memoryByteIndex, valueOut)',
    memory: 'write',
  },
  mstore8: {
    value: 0x53,
    delta: 2,
    alpha: 0,
    doc: 'mstore8(memoryByteIndex, valueOut [ & 0xff ])',
    memory: 'write',
  },
  ...
  sstore: {
    ...
    delta: 2,
    alpha: 0,
    doc: 'sstore(storageWordIndex, valueOut)',
    nonStatic: true,
  },
